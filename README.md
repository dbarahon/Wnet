...
To ease its implementation in global models, Wnet has been translated into a standalone fortran 90 module called Wnet and saved as Wnet.F90. Wnet_driver.F90 shows and example of the implementation using only Fortran. The module requires the file Wnet_weights.txt which stores the weights of the NN. Wnet has been optimized for openmp and could be compiled following gfortran -fopenmp -O3 -traceback -o Wnet.exe Wnet.F90 Wnet_driver.F90. The program PyF90_comparison.py compares the F90 and Python versions of Wnet.   
